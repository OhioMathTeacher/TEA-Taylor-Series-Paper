#!/usr/bin/env python3
"""
Batch runner to convert a folder of transcript files (.txt/.docx) into
single LaTeX table rows using your Appendix B prompt.

Usage examples:
  python3 batch_rows.py --input "." --limit 10 --model gpt-4o
  python3 batch_rows.py --limit 25 --force

Outputs:
  out_rows/rows.tex         (concatenated LaTeX rows)
  out_rows/rows.csv         (filename,row,status)
  out_rows/responses_raw/*  (raw model responses per file)
"""
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Output only a single LaTeX table row in the exact requested column order. "
                "Do not echo the header. Values must be plain numbers where applicable, end with '\\\\'."
            ),
        },
        {
            "role": "user",
    ...
    with csv_path.open("a", newline="", encoding="utf-8") as fcsv, tex_path.open("a", encoding="utf-8") as ftex:
        ...
        for i, path in enumerate(files, 1):
            ...
            try:
                ...
                try:
                    student_pct = row.strip().rstrip("\\").split("&")[-1].strip()
                except Exception:
                    student_pct = "?"
