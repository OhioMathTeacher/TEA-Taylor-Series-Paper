...
def screen_file(path, annotate_dir, units, approx, model=None, cls_idx=None, thresh=0.65):
    raw = load_text(path)
...
def main():
    ...
    ap.add_argument(
        ...
        default="words",
        help="Counting mode: words, chars, or both (default: words)"
    )
    ...
    with open(summary_path, "w", newline="", encoding="utf-8") as fsum, \
        ...
        for i, path in enumerate(files, 1):
            try:
                rec = screen_file(path,
                  (out_dir / "annotated") if args.annotate else None,
                  args.units, args.approx_words_from_chars,
